...
module.exports.deleteSubcategoryRecord=async(req,res)=>{
    try{
        const deteleSubcategory = await subcategory_model.findById(req.params.id);

        if (deteleSubcategory) {
            var ipath = path.join(__dirname,'..',deteleSubcategory.subcategoryimage)
            await fs.unlinkSync(ipath);
        }
    
        await subcategory_model.findByIdAndDelete(req.params.id);
        return res.redirect('back');
    }catch(err){
    console.log(err)
    return res.redirect('back')
